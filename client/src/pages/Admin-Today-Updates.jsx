...
export const AdminTodayUpdates = () => {
    ...
    const getAllTodayUpdatesData = async () => {
        try {
            const response = await fetch("http://localhost:5000/api/admin/today-updates", {
                method: "GET",
                headers: {
    ...
    const deleteCategory = async(id) => {
        try {
            const response = await fetch(`http://localhost:5000/api/admin/today-updates/delete/${id}`,{
                method: "DELETE",
                headers: {
                    Authorization: authorizationToken,
                }
